...
export async function getData<T>(collectionName: string, page = 1, pageSize = 10) {
    try {
        ...
        if (page > 1) {
            ...
            q = query(collection(db, collectionName), orderBy('createdAt', 'desc'), startAfter(lastDocument), limit(pageSize));
        }
